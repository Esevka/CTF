	...
	for x in range(length_tablas): 
		...
		with open('diccionario.txt','r') as dic:
			...
			for caracter in dic:
				...
				#comprobamos la posicion(index) en la que se encuentran las dos fechas en la respuesta del servidor
				index = r.text.find("2023-08-01")
				index1 = r.text.find("2023-08-02")

				#comparamos para saber si esta ordenando por title o fecha, de este modo sabemos si la comparativa se cumple o no y si es el caracter correcto o no.
				if index > index1:
	...
	for x in range(38): 
		...
		with open('diccionario.txt','r') as dic:
			...
			for caracter in dic:
				...
				#comprobamos la posicion(index) en la que se encuentran las dos fechas en la respuesta
				index = r.text.find("2023-08-01")
				index1 = r.text.find("2023-08-02")

				#comparamos para saber si esta ordenando por title o fecha, de este modo sabemos si la comparativa se cumple o no y si es el caracter correcto.
				if index > index1:
					print (letra,end="",flush=True)
					break
